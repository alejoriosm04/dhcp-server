...
int load_network_config(const char* filename, char* subnet_mask,
    char* default_gateway, char* dns_server, int* lease_time) {
...
int generate_ip_pool(const char *ip_start, const char *ip_end) {
    ...
    for (unsigned long ip = start; ip <= end && count < POOL_SIZE; ++ip) {
        ...
        memset(lease_table[count].mac_address, 0,
            sizeof(lease_table[count].mac_address));
...
void register_lease(lease_record* lease, const char* mac_address) {
    lease->lease_start = time(NULL);
    lease->assigned = 1;
    strcpy(lease->mac_address, mac_address);

    char log_entry[BUFFER_SIZE];
    snprintf(log_entry, BUFFER_SIZE,
        "Lease registrado para la IP %s con MAC %s por %ld segundos",
        lease->ip, mac_address, lease->lease_duration);
...
void renew_lease(lease_record* lease) {
    ...
    char log_entry[BUFFER_SIZE];
    snprintf(log_entry, BUFFER_SIZE,
        "Lease renovado para la IP %s con MAC %s por %ld segundos",
    ...
    for (int i = 0; i < POOL_SIZE; ++i) {
        if (strcmp(lease_table[i].ip, ip) == 0) {
            if (strcmp(lease_table[i].mac_address, mac_address) == 0) {
                ...
                memset(lease_table[i].mac_address, 0,
                    sizeof(lease_table[i].mac_address));

                char log_entry[BUFFER_SIZE];
                snprintf(log_entry, BUFFER_SIZE,
                    "IP %s liberada y disponible para nuevos clientes", ip);
                log_message("INFO", log_entry);
                ...
                printf("La MAC %s no coincide con el registro para la IP %s\n",
                    mac_address, ip);
                log_message("WARNING",
                    "Intento de liberar una IP con una MAC que no coincide.");
            }
            break;
        }
    }
...
void check_expired_leases() {
    ...
    for (int i = 0; i < POOL_SIZE; ++i) {
        if (lease_table[i].assigned &&
            difftime(current_time, lease_table[i].lease_start) >=
            lease_table[i].lease_duration) {
            lease_table[i].assigned = 0;
            lease_table[i].lease_start = 0;
            lease_table[i].lease_duration = 0;
            memset(lease_table[i].mac_address, 0,
            ...
            char log_entry[BUFFER_SIZE];
            snprintf(log_entry, BUFFER_SIZE,
                "Lease expirado para la IP %s. Liberando la dirección.",
                lease_table[i].ip);
            log_message("INFO", log_entry);
...
int main(int argc, char *argv[]) {
    if (argc != 4) {
        printf("Uso: %s <IP inicio> <IP fin> <archivo de configuración>\n",
            argv[0]);
        log_message("ERROR",
            "Uso incorrecto del servidor DHCP. Se requieren las IP de inicio, fin y el archivo de configuración.");
        return EXIT_FAILURE;
    }
    ...
    if (load_network_config(argv[3], subnet_mask, default_gateway,
        dns_server, &lease_time) != 0) {
    ...
    // Inicializar los parámetros de red en cada registro
    for (int i = 0; i < POOL_SIZE; ++i) {
        lease_table[i].lease_duration = lease_time;
    ...
    if (bind(udp_socket, (struct sockaddr *)&server_addr,
        ...
        perror("No se pudo enlazar el socket al puerto 67");
    ...
    while (1) {
        ...
        int bytes_received = recvfrom(udp_socket, request->buffer,
            BUFFER_SIZE, 0, (struct sockaddr *)&request->client_addr,
